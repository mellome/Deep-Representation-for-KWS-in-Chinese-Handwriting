...
from utils.gen_random_idx import get_random_list
...
def map_from_query_test_feature_matrices(query_features, test_features,
    ...
    # length_of_test = len(query_labels)
    # TODO yhe: set number of random queries
    query_features, query_labels = get_random_list(query_features, query_labels, 5000)

    print("<<<<<<<<<<<< query_features after randomisation")
    print(type(query_features))
    print(query_features.shape)
    print(query_features)
    print(">>>>>>><>>>>")

    print("<<<<<<<<<<<< test_features after randomisation")
    print(type(test_features))
    print(test_features.shape)
    print(test_features)
    print(">>>>>>><>>>>")
    ...
    if test_features.shape[0] > 5000:
        print("dimension of test_feature greater than 5000")
        ...
        print("dimension of test_feature smaller than 5000")
    ...
    print("<<<<<<<<<<<< retr_mat combined with row_selector")
    print(type(retr_mat))
    print(retr_mat.shape)
    print(retr_mat)
    print(">>>>>>><>>>>")
...
def complete_map_from_qry_test_list(query_features, test_features,
    ...
    # query_features = query_features[:5000, ]
    # query_labels = query_labels[:5000, ]
    avg_precs = np.zeros(1)

    if query_features.shape[0] > 5000:
        # initialize dist_mat: [size_of_query_features, 1]
        tmp_query_features = query_features
        tmp_query_labels = query_labels
        ...
        while size_of_tmp_query_features > 0:
            ...
            if size_of_tmp_query_features > 5000:
                avg_prec = calculate_avg_precs(tmp_query_features[:5000, ],
                                               tmp_query_labels[:5000, ],
                                               test_features,
                                               test_labels, drop_first)

                tmp_query_features = tmp_query_features[5000:, ]
                tmp_query_labels = tmp_query_labels[5000:, ]
                size_of_tmp_query_features = tmp_query_features.shape[0]
            else:
                avg_prec = calculate_avg_precs(tmp_query_features,
                                               tmp_query_labels,
                                               test_features,
                                               test_labels,
                                               drop_first)
            ...
            print("<<<<<<<<<<<< avg_precs is ready to concatenate")
            print(type(avg_precs))
            print(avg_precs.shape)
            print(type(avg_prec))
            print(avg_prec.shape)
            print(">>>>>>><>>>>")
            avg_precs = np.concatenate((avg_precs, avg_prec), axis=0)

    avg_precs = np.delete(avg_precs, 0, axis=0)
    print("<<<<<<<<<<<< avg_precs after hstack")
    print(type(avg_precs))
    print(avg_precs.shape)
    print(avg_precs)
    print(">>>>>>><>>>>")
...
def calculate_avg_precs(query_features, query_labels, test_features, test_labels, drop_first):
    # split feature vector over 5000+ dimensions
    if test_features.shape[0] > 5000:
        # initialize dist_mat: [size_of_query_features, 1]
        dist_mat = np.zeros((query_features.shape[0], 1))
        tmp_test_features = test_features
        ...
        while size_of_tmp_test_features > 0:
            ...
            # -> dist_mat: [size_of_query_features, m+n]
            # dist_mat = np.concatenate([dist_mat, tmp_dist_mat], axis=1)
            print("<<<<<<<<<<<< dist_mat before hstack <<for safety :-) >>")
            print(type(dist_mat))
            print(dist_mat.shape)
            print(dist_mat)
            print(type(tmp_dist_mat))
            print(tmp_dist_mat.shape)
            print(tmp_dist_mat)
            print(">>>>>>><>>>>")
            dist_mat = np.hstack((dist_mat, tmp_dist_mat))
