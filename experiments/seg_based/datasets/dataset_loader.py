...
from utils.character_container import SimpleCharacterContainer
from utils.gnt_converter import load_gnt
...
class DatasetLoader(object):  # pylint: disable=abstract-class-not-used
    ...
    def load_rendered_gw_online(path='/vol/corpora/document-image-analysis/gw-online', stroke_width=10,
        ...
        for img_file in tqdm.tqdm(
            sorted(os.listdir(os.path.join(path, 'renderedImages_sw%d_scale1.00' % stroke_width)))):
    ...
    def load_ifnenit(path, train_sets, test_sets, transcription_file=None):
        ...
        for transcrition_line in tqdm.tqdm(transcription_lines):
            ...
            if file_name[0] in train_sets:
                ...
                raise ValueError(
                    'The given image file %s can not be put in either train list or test list' % (file_name))
    ...
    def load_iamon(path='/vol/corpora/document-image-analysis/iam-db/', stroke_width=20, scale_factor=0.3):
        ...
        if not os.path.exists(img_dir_path):
            raise ValueError('The rendered IAM-OnDB does not exist for stroke_width %d and scale_factor %f' % (
    ...
    def load_icfhr2016_competition(dataset_name,
        ...
        # train
        train_list = DatasetLoader.load_from_READ_xml(
            xml_filename='_'.join((dataset_name, train_set, 'WL_CASE_INSENSITIVE.xml')),
        ...
        if test_annotation == 'SegFree':
            ...
            test_list = DatasetLoader.load_from_READ_xml(
                xml_filename='_'.join((dataset_name, 'Test_GT_SegFree_QbS.xml')),
            ...
            test_list = DatasetLoader.load_from_READ_xml(
                xml_filename='_'.join((dataset_name, 'Test', 'WordImages', 'WL_DUMMY.xml')),
                img_dir='_'.join((dataset_name, 'Test', 'WordImages')),
                dataset_name=dataset_name,
                path=path)
        # queries
        qry_list = DatasetLoader.load_from_READ_xml(
            xml_filename='_'.join((dataset_name, 'Test', 'QryImages', 'WL_DUMMY.xml')),
    ...
    def load_hw_synth_10k(split, path='/vol/big/ssudholt/hw-synth'):
        ...
        word_list = np.array([SimpleWordContainer(transcription=transcription.decode('ascii'),
                                                  bounding_box={'upperLeft': np.zeros(2, dtype=np.int32),
                                                                'widthHeight': np.ones(2, dtype=np.int32) * -1},
                                                  image_path=os.path.join(path, 'Images_90K_Normalized',
                                                                          img_path.decode('ascii')))
    ...
    @staticmethod
    def load_casia(path='/vol/corpora/document-image-analysis/casia', container_class=SimpleCharacterContainer):

        logger = logging.getLogger('DatasetLoader::CASIA')
        logger.info('Loading CASIA from %s...', path)

        # acquire all folder paths ['Gnt1-1', 'Gnt1-1-2',...]
        folder_files = [folder_name for folder_name in os.listdir(path)
                        if 'Gnt' in folder_name]

        # load the characters
        character_obj_list = []
        code_map = {}

        # extract the folders from casia folder
        for folder_file in tqdm.tqdm(folder_files):
            folder_id = folder_file[-1:]
            folder_path = os.path.join(path, folder_file)  # ../casia/Gnt1-1

            # acquire all gnt paths ['<name>.gnt', '<name>.gnt',...]
            gnt_files = [file_name for file_name in os.listdir(folder_path)
                         if file_name.endswith('.gnt')]

            # extract the segmented characters from every single folder
            for gnt_file in tqdm.tqdm(gnt_files):
                gnt_id = gnt_file.split('.')[0]
                gnt_path = os.path.join(folder_path, gnt_file)  # ../casia/Gnt1-1/<name>.gnt

                try:
                    character_list, label_list = load_gnt(gnt_path, gnt_id, code_map)
                except Exception as e:
                    print('>>>>> ' + gnt_path + ' : ' + gnt_id)
                    print(e)

                if len(character_list) != len(label_list):
                    raise ValueError('amount of character and amount of label are not same!')

                for character_np, label_int in zip(character_list, label_list):
                    # character, label, gnt_id, folder_id):
                    character_obj = container_class(character=character_np, label=label_int,
                                                    gnt_id=gnt_id, folder_id=folder_id)
                    character_obj_list.append(character_obj)
        return character_obj_list, code_map


if __name__ == '__main__':
    zip_dir = '/Users/mellome1992/Documents/LocalRepository/phocnet/src/gnt_utils/dataset'
    dst_dir = '/Users/mellome1992/Documents/LocalRepository/phocnet/experiments/seg_based/character_set'
    ...
    # character_elements = DatasetLoader.load_CASIA(zip_dir, dst_dir, CharacterContainer)
    character_elements, code_map = DatasetLoader.load_casia(zip_dir)

    character_instance = character_elements[3]

    character = character_instance.get_character()
    label = character_instance.get_label()
